...
void CartoonMaterial::_lazyInitializeShader()
{
	...
	_uLevelsHandle = _shader->getUniformLocation("levels");

	_initializeLightUniforms();
}

CartoonMaterial::CartoonMaterial(glm::vec3 pDiffuseColor, int pLevels)
	:	Material("cartoon"),
...
void CartoonMaterial::render(Mesh* pMesh, glm::mat4* pMatrix, glm::mat4* pModel, glm::mat4* pView, glm::mat4* pProjection)
{
	_shader->use();
	...
	_shader->setUniform(_uLevelsHandle, &_levels);
