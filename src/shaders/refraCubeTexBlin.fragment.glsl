...
void main()
{
	...
	vec4 reflectColor = texture(cubemapTexture, reflectedDirection);
	vec4 refractColor = texture(cubemapTexture, refractedDirection);
	textureColor = vec4(textureColor.rgb * enlightment, textureColor.a);
	reflectColor = vec4(reflectColor.rgb * enlightment, reflectColor.a);
	refractColor = vec4(refractColor.rgb, refractColor.a);
	fragmentColor = mix(mix(refractColor, reflectColor, reflectionFactor), textureColor, reflectionFactor);
}
